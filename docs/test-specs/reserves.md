...
Source CSV row range: 522–534

---

### Test 522: Review changes from PY if either >PM or 30% and obtain explanation for movement
...
### Test 523: Obtain the opening balance of retained earnings from prior year audited financial statements.
...
**Steps:**
 ...
 1. `ai_analysis`  — bindings: `{ prompt_template: 'Obtain the opening balance of retained earnings from prior year audited financial statements.', output_format: 'pass_fail' }`
...
### Test 524: Reconcile movements in retained earnings during the year with the general ledger, including profit or loss, di
...
> Reconcile movements in retained earnings during the year with the general ledger, including profit or loss, dividends, and prior period adjustments.
...
### Test 525: Verify that retained earnings are properly classified within equity.
...
**Steps:**
 ...
 1. `ai_analysis`  — bindings: `{ prompt_template: 'Check classification of Reserves transactions for correct sub-categorisation against policy.', output_format: 'pass_fail' }`
 2. `team_review`  — bindings: `{ instructions: 'Review AI assessment and conclude.' }`
...
### Test 526: Check for correct separation from other reserves, share capital, and other components of equity.
...
### Test 527: Review presentation of retained earnings in the statement of changes in equity and notes.
...
### Test 528: Check disclosures regarding dividends, transfers to reserves, and any restrictions on retained earnings.
...
### Test 529: Inspect reconciliations from the opening balance to the closing balance.
...
**Steps:**
 ...
 1. `ai_analysis`  — bindings: `{ prompt_template: 'Inspect reconciliations from the opening balance to the closing balance.', output_format: 'pass_fail' }`
 2. `team_review`  — bindings: `{ instructions: 'Review AI assessment and conclude.' }`
...
### Test 530: Verify that movements in retained earnings are supported by audited profit or loss accounts and dividend recor
...
> Verify that movements in retained earnings are supported by audited profit or loss accounts and dividend records.

**Steps:**

 1. `ai_analysis`  — bindings: `{ prompt_template: 'Verify that movements in retained earnings are supported by audited profit or loss accounts and dividend records.', output_format: 'pass_fail' }`
 2. `team_review`  — bindings: `{ instructions: 'Review AI assessment and conclude.' }`
...
### Test 531: Review calculations of profit or loss allocated to retained earnings, dividends declared, and prior period adj
...
> Review calculations of profit or loss allocated to retained earnings, dividends declared, and prior period adjustments.
...
### Test 532: Check for any overstatements or misstatements affecting equity.
...
**Steps:**
 ...
 1. `ai_analysis`  — bindings: `{ prompt_template: 'Check for any overstatements or misstatements affecting equity.', output_format: 'pass_fail' }`
 2. `team_review`  — bindings: `{ instructions: 'Review AI assessment and conclude.' }`
...
### Test 533: Verify whether any portion of retained earnings is subject to legal, contractual, or regulatory restrictions.
...
### Test 534: Review board minutes for dividend approvals and transfers to reserves.
