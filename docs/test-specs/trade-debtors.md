# Test Specs — Trade debtors

Framework: FRS102   Significant Risk: Y   Test count: 18

Source CSV row range: 297–314
...
- Output Format: `three_section_sampling`
- Execution Mode: `action_pipeline`
- Pattern: `subs_receipts` — Inspect subsequent receipts post-YE.

**Steps:**

 1. `review_subsequent_activity` 🆕 — bindings: `{ transaction_type: 'receipts', fs_line: $ctx.test.fsLine, x_days_post_ye: 60 }`
...
**Steps:**
 ...
 1. `ai_analysis`  — bindings: `{ prompt_template: 'Check classification of Trade debtors transactions for correct sub-categorisation against policy.', output_format: 'pass_fail' }`
...
**Steps:**
 ...
 1. `ai_analysis`  — bindings: `{ prompt_template: 'Check that receivables are recorded net of any allowances.', output_format: 'pass_fail' }`
 2. `team_review`  — bindings: `{ instructions: 'Review AI assessment and conclude.' }`
...
### Test 313: Inspect contracts, invoices, and sales agreements to confirm the entity’s right to receive payment.
