# Test Specs — Deferred revenue

Framework: FRS102   Significant Risk: Y   Test count: 16

Source CSV row range: 479–494

---

### Test 479: Review changes from PY if either >PM or 30% and obtain explanation for movement

- FS Line: **Deferred revenue**
...
### Test 480: Obtain a complete schedule of deferred revenue balances and reconcile with the general ledger.
...
### Test 481: Review customer contracts, invoices, and payment schedules to ensure all unearned income is recorded.
...
**Steps:**
 ...
 1. `ai_analysis`  — bindings: `{ prompt_template: 'Review customer contracts, invoices, and payment schedules to ensure all unearned income is recorded.', output_format: 'pass_fail' }`
...
### Test 482: Inspect post-year-end revenue recognition to identify unrecorded deferrals.
...
**Steps:**
 ...
 1. `ai_analysis`  — bindings: `{ prompt_template: 'Inspect post-year-end revenue recognition to identify unrecorded deferrals.', output_format: 'pass_fail' }`
 2. `team_review`  — bindings: `{ instructions: 'Review AI assessment and conclude.' }`
...
### Test 483: Verify that deferred revenue is correctly classified as a current or non-current liability depending on when i
...
> Verify that deferred revenue is correctly classified as a current or non-current liability depending on when it will be recognized.

**Steps:**

 1. `ai_analysis`  — bindings: `{ prompt_template: 'Check classification of Deferred revenue transactions for correct sub-categorisation against policy.', output_format: 'pass_fail' }`
 2. `team_review`  — bindings: `{ instructions: 'Review AI assessment and conclude.' }`
...
### Test 484: Check separation from trade payables or accrued income.
...
### Test 485: Review disclosures relating to deferred revenue, including nature, amounts, and timing of recognition.
...
### Test 486: Ensure deferred revenue is presented separately from other liabilities.
...
### Test 487: Inspect supporting contracts, invoices, and payment records.
...
### Test 488: Confirm customer payments that relate to deferred revenue exist.
...
**Steps:**
 ...
 1. `ai_analysis`  — bindings: `{ prompt_template: 'Confirm customer payments that relate to deferred revenue exist.', output_format: 'pass_fail' }`
 2. `team_review`  — bindings: `{ instructions: 'Review AI assessment and conclude.' }`
...
### Test 489: Perform cutoff testing to verify balances relate to the correct reporting period.
...
### Test 490: Recalculate deferred revenue based on contract terms, payment received, and revenue recognition policy.
...
**Steps:**
 ...
 1. `ai_analysis`  — bindings: `{ prompt_template: 'Recalculate deferred revenue based on contract terms, payment received, and revenue recognition policy.', output_format: 'pass_fail' }`
 2. `team_review`  — bindings: `{ instructions: 'Review AI assessment and conclude.' }`
...
### Test 491: Ensure amounts are recorded accurately and consistently.
...
**Steps:**
 ...
 1. `ai_analysis`  — bindings: `{ prompt_template: 'Ensure amounts are recorded accurately and consistently.', output_format: 'pass_fail' }`
 2. `team_review`  — bindings: `{ instructions: 'Review AI assessment and conclude.' }`
...
### Test 492: Review any discounts, rebates, or performance obligations that affect recognition.
...
**Steps:**
 ...
 1. `ai_analysis`  — bindings: `{ prompt_template: 'For Deferred revenue, assess how the performance obligation is satisfied per the contract terms.', output_format: 'pass_fail' }`
 2. `team_review`  — bindings: `{ instructions: 'Review AI assessment and conclude.' }`
...
### Test 493: Inspect contracts and agreements to confirm the entity has an obligation to deliver goods or services in excha
...
> Inspect contracts and agreements to confirm the entity has an obligation to deliver goods or services in exchange for the deferred revenue.
...
### Test 494: Check whether there are any restrictions or conditions affecting recognition or repayment.
