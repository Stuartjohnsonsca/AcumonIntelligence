# Test Specs — Interest payable and similar income

Framework: FRS102   Significant Risk: Y   Test count: 22

Source CSV row range: 141–162
...
### Test 142: Obtain an understanding of the entity’s financing arrangements (bank loans, overdrafts, lease liabilities, int
...
> Obtain an understanding of the entity’s financing arrangements (bank loans, overdrafts, lease liabilities, intercompany borrowings).

**Steps:**

 1. `ai_analysis`  — bindings: `{ prompt_template: 'Obtain an understanding of the entity’s financing arrangements (bank loans, overdrafts, lease liabilities, intercompany borrowings).', output_format: 'pass_fail' }`
...
### Test 144: Perform analytical review by comparing actual interest charges against expectations (loan balance × effective 
...
> Perform analytical review by comparing actual interest charges against expectations (loan balance × effective rate).

**Steps:**

 1. `ai_analysis`  — bindings: `{ prompt_template: 'Perform analytical review by comparing actual interest charges against expectations (loan balance × effective rate).', output_format: 'pass_fail' }`
 2. `team_review`  — bindings: `{ instructions: 'Review AI assessment and conclude.' }`
...
**Steps:**
 ...
 1. `ai_analysis`  — bindings: `{ prompt_template: 'Inspect post year-end payments of interest to confirm completeness.', output_format: 'pass_fail' }`
 2. `team_review`  — bindings: `{ instructions: 'Review AI assessment and conclude.' }`
...
**Steps:**
 ...
 1. `ai_analysis`  — bindings: `{ prompt_template: 'Verify that expenses recorded relate to genuine liabilities of the entity.', output_format: 'pass_fail' }`
 2. `team_review`  — bindings: `{ instructions: 'Review AI assessment and conclude.' }`
...
### Test 151: Assess appropriateness of management’s estimates where accrued interest or amortised costs are used.
...
**Steps:**
 ...
 1. `ai_analysis`  — bindings: `{ prompt_template: 'Check classification of Interest payable and similar income transactions for correct sub-categorisation against policy.', output_format: 'pass_fail' }`
 2. `team_review`  — bindings: `{ instructions: 'Review AI assessment and conclude.' }`
