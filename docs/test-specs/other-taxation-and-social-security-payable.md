# Test Specs — Other taxation and social security payable

Framework: FRS102   Significant Risk: Y   Test count: 15

Source CSV row range: 449–463
...
**Steps:**
 ...
 1. `ai_analysis`  — bindings: `{ prompt_template: 'Review VAT returns, PAYE/NIC submissions, and tax correspondence to ensure all liabilities are recorded.', output_format: 'pass_fail' }`
...
**Steps:**
 ...
 1. `ai_analysis`  — bindings: `{ prompt_template: 'Check classification of Other taxation and social security payable transactions for correct sub-categorisation against policy.', output_format: 'pass_fail' }`
 2. `team_review`  — bindings: `{ instructions: 'Review AI assessment and conclude.' }`
...
### Test 455: Review disclosures in the notes to ensure “Other taxation and social security” is presented separately from co
...
> Review disclosures in the notes to ensure “Other taxation and social security” is presented separately from corporation tax and trade creditors.
...
**Steps:**
 ...
 1. `ai_analysis`  — bindings: `{ prompt_template: 'Inspect VAT returns, HMRC correspondence, PAYE/NIC schedules, and other tax filings to verify balances.', output_format: 'pass_fail' }`
 2. `team_review`  — bindings: `{ instructions: 'Review AI assessment and conclude.' }`
...
**Steps:**
 ...
 1. `ai_analysis`  — bindings: `{ prompt_template: 'Trace recorded liabilities to underlying tax returns or payroll records.', output_format: 'pass_fail' }`
 2. `team_review`  — bindings: `{ instructions: 'Review AI assessment and conclude.' }`
...
**Steps:**
 ...
 1. `ai_analysis`  — bindings: `{ prompt_template: 'Recalculate VAT payable or receivable from underlying accounting records.', output_format: 'pass_fail' }`
 2. `team_review`  — bindings: `{ instructions: 'Review AI assessment and conclude.' }`
...
**Steps:**
 ...
 1. `ai_analysis`  — bindings: `{ prompt_template: 'Reconcile PAYE/NIC amounts to payroll records.', output_format: 'pass_fail' }`
 2. `team_review`  — bindings: `{ instructions: 'Review AI assessment and conclude.' }`
...
**Steps:**
 ...
 1. `ai_analysis`  — bindings: `{ prompt_template: 'Check provisions for interest/penalties if returns are late or under dispute.', output_format: 'pass_fail' }`
 2. `team_review`  — bindings: `{ instructions: 'Review AI assessment and conclude.' }`
...
### Test 462: Review correspondence with HMRC and confirm the entity’s legal obligations.
