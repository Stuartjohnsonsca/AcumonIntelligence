...
export async function GET(_req: Request, { params }: { params: Promise<{ engagementId: string }> }) {
  ...
  const row = await prisma.auditTaxOnProfits.findUnique({ where: { engagementId } });
...
/**
 * Shallow-merge PUT — same merge semantics as the VAT reconciliation
 * handler. Pass `{ replace: true }` to overwrite the entire blob.
 */
