...
export async function GET(req: Request, { params }: { params: Promise<{ engagementId: string }> }) {
  ...
  if (url.searchParams.get('meta') === 'signoffs') {
    return getJsonTableSignOffs(prisma.auditEthics, engagementId);
  }

  const ethics = await prisma.auditEthics.findUnique({ where: { engagementId } });
  return NextResponse.json({ data: ethics?.data || {} });
}
...
export async function PUT(req: Request, { params }: { params: Promise<{ engagementId: string }> }) {
  ...
  if (body.fieldMeta) {
    await saveJsonTableFieldMeta(prisma.auditEthics, engagementId, body.fieldMeta);
  }

  await prisma.auditEthics.upsert({
    where: { engagementId },
...
export async function POST(req: Request, { params }: { params: Promise<{ engagementId: string }> }) {
  ...
  if (body.action === 'signoff') {
    return handleJsonTableSignOff(prisma.auditEthics, engagementId, {
      engagementId,
