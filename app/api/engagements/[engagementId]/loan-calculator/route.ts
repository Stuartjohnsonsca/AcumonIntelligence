...
/**
 * Loan Calculator load/save endpoint.
 *
 * GET returns the engagement's loan-calculator JSON blob (or {} if no
 * row exists yet). PUT shallow-merges the supplied `data` slice into the
 * existing row, mirroring the VAT Reconciliation handler. Pass
 * { replace: true } to overwrite the whole blob.
 */
...
export async function GET(_req: Request, { params }: { params: Promise<{ engagementId: string }> }) {
  ...
  const row = await (prisma as any).auditLoanCalculator?.findUnique({ where: { engagementId } });
  return NextResponse.json({
    data: row?.data || {},
    clientName: eng.client?.clientName || '',
    periodStartDate: eng.period?.startDate || null,
    periodEndDate: eng.period?.endDate || null,
    priorEngagementId: eng.priorPeriodEngagementId || null,
  });
}
...
export async function PUT(req: Request, { params }: { params: Promise<{ engagementId: string }> }) {
  ...
  const existingData = (existing?.data && typeof existing.data === 'object' && !Array.isArray(existing.data))
    ...
    : {};
  const merged: Record<string, unknown> = replace ? { ...data, updatedAt: new Date().toISOString() } : { ...existingData, ...data, updatedAt: new Date().toISOString() };
