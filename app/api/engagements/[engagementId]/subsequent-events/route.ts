...
export async function GET(req: Request, { params }: { params: Promise<{ engagementId: string }> }) {
  ...
  if (url.searchParams.get('meta') === 'signoffs') {
    return getJsonTableSignOffs(prisma.auditSubsequentEvents, engagementId);
  }

  const record = await prisma.auditSubsequentEvents.findUnique({ where: { engagementId } });
...
export async function PUT(req: Request, { params }: { params: Promise<{ engagementId: string }> }) {
  ...
  if (body.fieldMeta) {
    await saveJsonTableFieldMeta(prisma.auditSubsequentEvents, engagementId, body.fieldMeta);
  }

  await prisma.auditSubsequentEvents.upsert({
    where: { engagementId },
...
export async function POST(req: Request, { params }: { params: Promise<{ engagementId: string }> }) {
  ...
  if (body.action === 'signoff') {
    return handleJsonTableSignOff(prisma.auditSubsequentEvents, engagementId, {
      engagementId,
  ...
  if (body.action === 'unsignoff') {
    return handleJsonTableUnsignOff(prisma.auditSubsequentEvents, engagementId, session.user.id, body.role);
  }
