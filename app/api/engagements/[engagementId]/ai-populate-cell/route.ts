...
(b) A JSON array of clickable references pointing at the tabs the reader should open. Each reference: { "tab": one of ${KNOWN_TABS.map(t => `"${t}"`).join(' | ')}, "anchor": optional string (row id / section key), "label": short display label }.
