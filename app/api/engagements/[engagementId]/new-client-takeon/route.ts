...
export async function GET(req: Request, { params }: { params: Promise<{ engagementId: string }> }) {
  ...
  if (url.searchParams.get('meta') === 'signoffs') {
    return getJsonTableSignOffs(prisma.auditNewClientTakeOn, engagementId);
  }

  const record = await prisma.auditNewClientTakeOn.findUnique({ where: { engagementId } });
...
export async function PUT(req: Request, { params }: { params: Promise<{ engagementId: string }> }) {
  ...
  if (body.fieldMeta) {
    await saveJsonTableFieldMeta(prisma.auditNewClientTakeOn, engagementId, body.fieldMeta);
  }

  await prisma.auditNewClientTakeOn.upsert({
    where: { engagementId },
...
export async function POST(req: Request, { params }: { params: Promise<{ engagementId: string }> }) {
  ...
  if (body.action === 'signoff') {
    return handleJsonTableSignOff(prisma.auditNewClientTakeOn, engagementId, {
      engagementId,
  ...
  if (body.action === 'unsignoff') {
    return handleJsonTableUnsignOff(prisma.auditNewClientTakeOn, engagementId, session.user.id, body.role);
  }
