...
export const SYSTEM_ACTIONS: ActionDefinitionData[] = [
...

  {
    ...
    description: 'Interrogates the engagement\'s trial balance and returns the subset of account codes matching the supplied criteria. Filters can combine FS line, code pattern (prefix / range / explicit list), description match, statement (P&L vs BS), balance sign / range, and the is_accrual_account flag. Every text/pattern/boolean filter has a paired "exclude" variant — supply both to say "every overhead code 5000-5999 EXCEPT 5100,5101 and not anything containing the word DRAFT". Excludes are applied AFTER includes, so an exclude can never widen the population. Outputs a comma-separated `account_codes` string ready to feed straight into Request Listing from Client / Reconcile to Trial Balance / Compare Interest & Fees, plus the matching TB rows for chaining or reporting. Use as the first step of a test that needs to operate against a dynamic group of GL codes (e.g. "every overhead-expense account with an in-period balance over £10k").',
    category: 'data',
    ...
    inputSchema: [
      ...
      { code: 'exclude_fs_lines', label: 'Exclude FS Lines', type: 'text', required: false, source: 'user', group: 'FS Mapping', description: 'Comma-separated FS-line names to drop from the result (case-insensitive exact match against the canonical name). Applied after the include filter — useful with "None" or "This test’s FS line" to carve out a sibling FS line you don’t want in the population.' },
      ...
      { code: 'exclude_fs_level', label: 'Exclude FS Level', type: 'text', required: false, source: 'user', group: 'FS Mapping', description: 'Same syntax as FS Level above — case-insensitive substring match, comma-separated for multiple values. Any TB row whose fs_level contains one of these tokens is dropped, even if it satisfied an include filter.' },
      ...
      { code: 'exclude_code_pattern', label: 'Exclude Code Pattern', type: 'text', required: false, source: 'user', group: 'Code Filter', description: 'Same syntax as Code Pattern above — supports wildcards, ranges, lists and semicolon-combined patterns. Codes matching this expression are dropped from the population.' },
      { code: 'description_contains', label: 'Description Contains', type: 'text', required: false, source: 'user', group: 'Code Filter', description: 'Case-insensitive substring match against the TB row description / account name. Comma-separated values are treated as OR (a row matches if it contains any of them).' },
      { code: 'description_excludes', label: 'Description Excludes', type: 'text', required: false, source: 'user', group: 'Code Filter', description: 'Case-insensitive substring exclusions — comma-separated for multiple values. Any row whose description contains one of these tokens is dropped (e.g. "DRAFT, Suspense, Test").' },
      ...
      { code: 'max_abs_balance_gbp', label: 'Maximum Absolute Balance (GBP)', type: 'number', required: false, source: 'user', group: 'Balance Filter', description: 'Drops rows whose |current year| exceeds this value. Leave blank for no upper cap.' },
      ...
      { code: 'exclude_accruals', label: 'Exclude Accrual Accounts', type: 'boolean', required: false, source: 'user', defaultValue: false, group: 'Code Filter', description: 'When on, drops every TB row flagged is_accrual_account. The opposite of "Restrict to Accrual Accounts" — useful when you want to test the non-accrual side of an FS line. Setting both to on returns no rows.' },
      { code: 'period_end', label: 'Period End', type: 'date', required: false, source: 'auto', autoMapFrom: '$ctx.engagement.periodEnd', group: 'Context' },
    ],
    outputSchema: [
...

  {
    ...
    inputSchema: [
      ...
      { code: 'verify_against_tb', label: 'Check response to Trial Balance', type: 'boolean', required: false, source: 'user', defaultValue: true, group: 'Reconciliation', description: 'When on (default), the action parses the returned listing on resume and SUMIFs each row by `code_column` against the TB total of the requested account codes. If the variance exceeds tolerance the pipeline stays paused and a follow-up outstanding is raised so the client can correct or explain the difference. Turn off when the response shape doesn\'t carry per-row codes (the Reconcile Listing to TB step downstream can then handle it explicitly).' },
      { code: 'code_column', label: 'Code Column on Listing', type: 'text', required: false, source: 'user', defaultValue: 'account_code', group: 'Reconciliation', description: 'Column on the returned listing that holds each row\'s TB account code. Only used when "Check response to TB" is on.' },
      { code: 'amount_column', label: 'Amount Column on Listing', type: 'text', required: false, source: 'user', defaultValue: 'amount', group: 'Reconciliation' },
      { code: 'account_codes', label: 'TB Account Codes (optional)', type: 'text', required: false, source: 'user', group: 'Reconciliation', description: 'Comma-separated codes the listing should reconcile to. Bind to `$prev.account_codes` when paired with Select TB Account Codes upstream. Leave blank when the test is scoped to a single FS line — the action falls back to every TB code mapped to that FS line.' },
    ...
    outputSchema: [
      ...
      { code: 'variance', label: 'Variance', type: 'number' },
      { code: 'tb_reconciled', label: 'Reconciled', type: 'pass_fail' },
      { code: 'reconciliation', label: 'Per-Code Reconciliation', type: 'data_table', description: 'Row per requested code: account_code, description, listing_total, tb_balance, variance, status (matched | variance | missing_in_listing).' },
      { code: 'missing_codes', label: 'Codes Missing from Listing', type: 'data_table' },
      { code: 'extra_codes', label: 'Extra Codes in Listing', type: 'data_table' },
      { code: 'portal_request_id', label: 'Portal Request ID', type: 'text' },
      ...DEDUP_OUTPUT_FIELDS,
    ],
  },
  ...
  {
    code: 'request_calculation',
    name: 'Request Calculation from Management',
    description: 'Asks management, via the portal, for a worked calculation supporting a balance — ECL provision, depreciation charge, interest accrual, goodwill impairment, deferred tax, lease liability, stock valuation, bad debt provision, corporation tax, pension cost, fair-value workings, etc. The message lists the components the audit needs to see (e.g. for depreciation: gross cost, prior accumulated, useful life, in-period additions/disposals, charge), the preferred file format, and any TB account codes the result should agree to. The dedup pre-check looks for a recent calculation we already have on file (engagement document library + prior portal responses) so we don\'t pester the client for something they\'ve already supplied. Output exposes the returned documents and chat_response in the same shape as request_documents — chains naturally into Recalculate Balance for an independent re-perform, or Extract & Verify Listing to TB for a line-by-line tie to the trial balance.',
    category: 'evidence',
    handlerName: 'requestCalculation',
    icon: 'Calculator',
    color: '#3b82f6',
    isSystem: true,
    inputSchema: [
      { code: 'calculation_type', label: 'Calculation Type', type: 'select', required: true, source: 'user', group: 'Request Details', options: [
        { value: 'ecl',                   label: 'Expected credit loss / bad-debt provision' },
        { value: 'depreciation',          label: 'Depreciation charge (FAR / PPE)' },
        { value: 'interest_accrual',      label: 'Interest accrual / loan interest' },
        { value: 'goodwill_impairment',   label: 'Goodwill / asset impairment' },
        { value: 'deferred_tax',          label: 'Deferred tax computation' },
        { value: 'corporation_tax',       label: 'Corporation tax computation' },
        { value: 'lease_liability',       label: 'Lease liability schedule (IFRS 16 / FRS 102 §20)' },
        { value: 'stock_valuation',       label: 'Stock / inventory valuation' },
        { value: 'pension_cost',          label: 'Pension cost / post-employment benefits' },
        { value: 'fair_value',            label: 'Fair value workings' },
        { value: 'share_based_payment',   label: 'Share-based payment charge' },
        { value: 'revenue_recognition',   label: 'Revenue recognition / variable consideration' },
        { value: 'provisions',            label: 'Other provisions (warranty, restructuring, etc.)' },
        { value: 'fx_translation',        label: 'FX translation' },
        { value: 'prepayment_amortise',   label: 'Prepayment amortisation' },
        { value: 'other',                 label: 'Other — specify in the message' },
      ]},
      { code: 'message_to_client', label: 'Message to Client', type: 'textarea', required: true, source: 'user', group: 'Request Details', defaultValue: 'Please provide your calculation supporting this balance, with workings shown. Include all inputs (assumptions, rates, ageing buckets, useful lives, etc.), the formula or methodology applied, and the resulting amount. An Excel or CSV with one row per component is ideal.', description: 'Portal message. Supports {{placeholders}} — a default is suggested from the calculation_type.' },
      { code: 'expected_components', label: 'Expected Components', type: 'multiselect', required: false, source: 'user', group: 'Request Details', defaultValue: ['inputs', 'methodology', 'result'], description: 'What the response must include for the audit team to verify it. Drives the wording of the portal request.', options: [
        { value: 'inputs',         label: 'Inputs / assumptions (rates, lives, periods, ageing buckets, etc.)' },
        { value: 'methodology',    label: 'Methodology / formula applied' },
        { value: 'result',         label: 'Resulting balance' },
        { value: 'period_split',   label: 'Period-by-period split (monthly / quarterly)' },
        { value: 'prior_period',   label: 'Prior-period comparative figure' },
        { value: 'reconciliation', label: 'Reconciliation to TB account codes' },
        { value: 'sensitivity',    label: 'Sensitivity / scenario analysis' },
      ]},
      { code: 'preferred_format', label: 'Preferred File Format', type: 'select', required: false, source: 'user', defaultValue: 'excel_csv', group: 'Request Details', options: [
        { value: 'excel_csv', label: 'Excel or CSV (preferred)' },
        { value: 'pdf',       label: 'PDF' },
        { value: 'word',      label: 'Word document' },
        { value: 'any',       label: 'Any format' },
      ]},
      { code: 'template_attachment', label: 'Template to Send (optional)', type: 'file', required: false, source: 'user', group: 'Request Details', description: 'Optional template file the client should fill in (e.g. firm\'s standard ECL workings spreadsheet). Attached to the portal request when supplied.' },
      { code: 'account_codes', label: 'TB Account Codes (optional)', type: 'text', required: false, source: 'user', group: 'TB Mapping', description: 'Comma-separated codes the calculation should reconcile to. Bind to `$prev.account_codes` when paired with Select TB Account Codes upstream — passed through on the output so a downstream Extract & Verify or Recalculate step can do the line-by-line check.' },
      { code: 'period_end', label: 'Period End', type: 'date', required: false, source: 'auto', autoMapFrom: '$ctx.engagement.periodEnd', group: 'Context' },
      { code: 'area_of_work', label: 'Area of Work', type: 'text', required: false, source: 'auto', autoMapFrom: '$ctx.test.fsLine', group: 'Context', description: 'Auto-populated from the FS Line being tested. Used as the evidence tag and for portal routing.' },
      ...DEDUP_INPUT_FIELDS,
    ],
    outputSchema: [
      { code: 'documents', label: 'Returned Documents', type: 'file_array', description: 'Files the client uploaded with their workings.' },
      { code: 'chat_response', label: 'Client Response', type: 'text', description: 'Free-text the client typed in the portal chat (numbers, comments, methodology).' },
      { code: 'data_table', label: 'Parsed Workings (when available)', type: 'data_table', description: 'When the response was a structured upload (Excel/CSV) and an upstream extractor parsed it, the rows land here ready for verification.' },
      { code: 'calculation_type', label: 'Calculation Type', type: 'text', description: 'Echoes the input — downstream verify steps switch on this.' },
      { code: 'account_codes', label: 'TB Account Codes', type: 'text', description: 'Echoes the input so downstream Extract & Verify / Recalculate steps can chain off `$prev.account_codes` without re-typing.' },
      { code: 'portal_request_id', label: 'Portal Request ID', type: 'text' },
      { code: 'outstanding_count', label: 'Outstanding', type: 'number', description: '1 while waiting for the client; 0 once dedup found a satisfying answer already on file.' },
      ...DEDUP_OUTPUT_FIELDS,
    ],
  },
  ...
  {
    code: 'extract_and_verify_listing',
    name: 'Extract & Verify Listing to TB',
    description: 'Single-step ingest + line-by-line verify. Takes whatever shape the management response arrived in — a parsed data_table from an upstream step, an Excel/CSV upload, or pasted text in the portal chat — normalises every line into a spreadsheet (account_code, description, amount, plus any extra columns the source carried), then walks row-by-row against the engagement\'s trial balance. Each line is matched to a TB code (exact code first, fuzzy description fallback) and the amount is compared to the code\'s current-year balance. Output is a verification table with one row per response line — status, matched TB code, listing_total, tb_balance, variance — plus an `unmatched_lines` table for response lines that don\'t map to any TB code, and a `missing_from_response` table for TB codes that should have been in the response but weren\'t (when the requested account_codes input is supplied). Drop in after Request Listing from Client to turn raw management input into an audit-ready, TB-verified table without a manual import step.',
    category: 'verification',
    handlerName: 'extractAndVerifyListing',
    icon: 'Scale',
    color: '#22c55e',
    isSystem: true,
    inputSchema: [
      { code: 'data_table', label: 'Pre-Parsed Response (preferred)', type: 'json_table', required: false, source: 'auto', autoMapFrom: '$prev.data_table', group: 'Source', description: 'When the previous step already parsed the response into a table, bind it here and the action skips re-parsing.' },
      { code: 'chat_response', label: 'Chat Response Text', type: 'textarea', required: false, source: 'auto', autoMapFrom: '$prev.chat_response', group: 'Source', description: 'Free-text response the client typed in the portal. Tab- or comma-separated rows are split into lines automatically; first non-blank row that looks like a header is used to label columns.' },
      { code: 'source_documents', label: 'Source Documents', type: 'file', required: false, source: 'auto', autoMapFrom: '$prev.documents', group: 'Source', description: 'Files the client uploaded. Reserved for AI extraction when neither data_table nor chat_response is supplied — falls back to a soft-fail when the extraction infra isn\'t wired in yet for this engagement.' },
      { code: 'code_column', label: 'Code Column', type: 'text', required: false, source: 'user', defaultValue: 'account_code', group: 'Mapping', description: 'Column on the parsed listing that holds each line\'s GL / account code. Falls back to the first column whose name matches /code|nominal|gl/i.' },
      { code: 'description_column', label: 'Description Column', type: 'text', required: false, source: 'user', defaultValue: 'description', group: 'Mapping', description: 'Column holding the account name / narrative. Used for fuzzy fallback when the code column doesn\'t match a TB row.' },
      { code: 'amount_column', label: 'Amount Column', type: 'text', required: false, source: 'user', defaultValue: 'amount', group: 'Mapping' },
      { code: 'match_strategy', label: 'Matching Strategy', type: 'select', required: false, source: 'user', defaultValue: 'code_then_description', group: 'Matching', options: [
        { value: 'code_then_description', label: 'Try code first, fall back to description' },
        { value: 'code_only',              label: 'Code only (strict)' },
        { value: 'description_only',       label: 'Description only (fuzzy)' },
      ]},
    ...
    outputSchema: [
      { code: 'data_table', label: 'Normalised Response', type: 'data_table', description: 'One row per line in the management response: original_row, code_supplied, description_supplied, amount_supplied, plus the resolved account_code / description from the TB.' },
      { code: 'verification', label: 'Verification (per line)', type: 'data_table', description: 'Per-line status: matched | amount_variance | no_tb_match. Carries listing_total, tb_balance, variance, match_via (code | description | none), match_score for fuzzy matches.' },
      { code: 'unmatched_lines', label: 'Unmatched Response Lines', type: 'data_table', description: 'Subset of the verification table where status = no_tb_match. Likely indicates a typo in the client\'s code column or a response line outside the requested scope.' },
      { code: 'missing_from_response', label: 'Codes Missing from Response', type: 'data_table', description: 'Codes listed in `account_codes` but with no matching line in the response. Empty when account_codes is not supplied.' },
      { code: 'matched_count', label: 'Matched Count', type: 'number' },
      { code: 'variance_count', label: 'Variance Count', type: 'number' },
      { code: 'unmatched_count', label: 'Unmatched Count', type: 'number' },
      { code: 'pass_fail', label: 'Overall Result', type: 'pass_fail' },
    ],
  },
  ...
  {
    code: 'apply_factor',
    name: 'Apply Factor / Multiplier',
    description: 'Scales an upstream balance by a factor — typically a rate (corporation-tax %, VAT %, commission %, interest %) or a unit multiplier (units × cost-per-unit). Pairs naturally with Combine / Aggregate Balances upstream: aggregate first to a single base figure, then multiply by the rate to get the expected charge / liability. Example chain — Aggregate DTA + DTL → Apply Factor (× corporation tax rate) → Reconcile to TB. Optional `expected_total` triggers a tolerance check so the action also emits `variance` + pass_fail without needing a separate verify step. The factor can be expressed as either a percentage (25 = 25%) or a raw multiplier (0.25), controlled by a single toggle.',
    category: 'verification',
    handlerName: 'applyFactor',
    icon: 'Calculator',
    color: '#22c55e',
    isSystem: true,
    inputSchema: [
      { code: 'base_label', label: 'Base Balance Label', type: 'text', required: false, source: 'user', defaultValue: 'Base balance', group: 'Base', description: 'Friendly name for the figure being scaled (e.g. "Net deferred tax", "Total revenue"). Echoed in the breakdown for reporting.' },
      { code: 'base_amount', label: 'Base Amount', type: 'number', required: true, source: 'auto', autoMapFrom: '$prev.total', group: 'Base', description: 'Bind to the upstream step\'s amount, e.g. `$prev.total` (Aggregate Balances) or `$step.N.<field>` to pick a specific output.' },
      { code: 'factor', label: 'Factor / Rate', type: 'number', required: true, source: 'user', group: 'Factor', description: 'The multiplier. Enter a literal number (25 for 25%, 0.25 for the raw rate, 1.05 for a 5% uplift) or bind to an upstream value, including a specific cell in a schedule — e.g. `$step.0.data_table.0.interest_rate` to use the first loan tranche\'s rate. When "Treat as percentage" is on, the value is divided by 100 before multiplying.' },
      { code: 'factor_as_percentage', label: 'Treat as a percentage', type: 'boolean', required: false, source: 'user', defaultValue: true, group: 'Factor', description: 'On: factor is divided by 100 before multiplying — typical for tax / commission / interest rates. Off: factor is used as-is — typical for unit multipliers.' },
      { code: 'factor_label', label: 'Factor Label', type: 'text', required: false, source: 'user', group: 'Factor', description: 'Friendly name for the factor (e.g. "UK corporation tax rate"). When blank, the breakdown uses the numeric factor.' },
      { code: 'expected_total', label: 'Expected Result (optional)', type: 'number', required: false, source: 'user', group: 'Verification', description: 'When supplied, the action compares `result` against this and emits `variance` + a pass/fail.' },
      { code: 'tolerance_gbp', label: 'Tolerance (GBP)', type: 'number', required: false, source: 'user', defaultValue: 1, group: 'Verification' },
      { code: 'account_codes', label: 'TB Account Codes (echo)', type: 'text', required: false, source: 'user', group: 'Chaining', description: 'Optional — echoed on the output so a downstream Reconcile to TB / Extract & Verify Listing step can chain off `$prev.account_codes` without re-typing.' },
    ],
    outputSchema: [
      { code: 'result', label: 'Result', type: 'number', description: 'base_amount × applied_factor.' },
      { code: 'total', label: 'Result (alias)', type: 'number', description: 'Same as `result`. Provided so this action can chain into Reconcile to TB / Extract & Verify Listing in the same shape as Aggregate Balances.' },
      { code: 'breakdown', label: 'Calculation Breakdown', type: 'data_table', description: 'Three rows showing the working: base, factor, result.' },
      { code: 'data_table', label: 'Calculation Breakdown (alias)', type: 'data_table' },
      { code: 'base_amount', label: 'Base Amount (echo)', type: 'number' },
      { code: 'applied_factor', label: 'Applied Factor', type: 'number', description: 'The factor actually used after percentage normalisation (e.g. 25 with the percentage toggle on becomes 0.25 here).' },
      { code: 'expected_total', label: 'Expected (echo)', type: 'number' },
      { code: 'variance', label: 'Variance vs Expected', type: 'number', description: 'result − expected_total. Null when expected_total is not supplied.' },
      { code: 'account_codes', label: 'TB Account Codes (echo)', type: 'text' },
      { code: 'pass_fail', label: 'Within Tolerance', type: 'pass_fail' },
    ],
  },
...

  {
    ...
    isSystem: true,
    inputSchema: [
      { code: 'policy_type', label: 'Policy Type', type: 'select', required: true, source: 'user', defaultValue: 'auto', group: 'Method', description: '"Auto" reads each row\'s `depreciation_method` and `useful_life` columns and applies the right formula per row — the only sensible choice for a Fixed Asset Register that mixes policies (motor vehicles reducing-balance + plant & machinery straight-line + etc.). Pick a fixed policy only when every row uses the same method.', options: [
        { value: 'auto', label: 'Auto (per-row from depreciation_method column)' },
        { value: 'straight_line_depreciation', label: 'Straight-line depreciation (whole population)' },
        { value: 'reducing_balance_depreciation', label: 'Reducing-balance depreciation (whole population)' },
      ...
      { code: 'inputs', label: 'Inputs', type: 'json_table', required: true, source: 'auto', autoMapFrom: '$prev.data_table', group: 'Data', description: 'For depreciation, the FAR rows should carry: cost (or gross_cost), useful_life (years), depreciation_method (straight_line / reducing_balance), acquisition_date, accumulated_depreciation_bf, depreciation_rate (% — used for reducing-balance), additions_in_period, disposals_in_period, disposal_date, salvage_value (optional). Column names are matched case-insensitively and several common variants are tolerated.' },
      { code: 'booked_charge_column', label: 'Booked Charge Column', type: 'text', required: false, source: 'user', defaultValue: 'charge_for_period', group: 'Data', description: 'Column on the input rows that holds the booked depreciation charge for the period — used for the variance comparison. Common values: charge_for_period, depreciation_charge, period_charge, charge.' },
    ...
    outputSchema: [
      { code: 'recalc_table', label: 'Recalculation', type: 'data_table', description: 'Per row: every input column passed through, plus policy_applied, calculated, booked, variance, pass_fail, notes.' },
      { code: 'data_table', label: 'Recalculation (alias)', type: 'data_table' },
      { code: 'total_variance', label: 'Total Variance', type: 'number' },
      { code: 'total_calculated', label: 'Total Calculated Charge', type: 'number' },
      { code: 'total_booked', label: 'Total Booked Charge', type: 'number' },
      { code: 'red_count', label: 'Rows Outside Tolerance', type: 'number' },
      { code: 'green_count', label: 'Rows Within Tolerance', type: 'number' },
      { code: 'unrecalculated_count', label: 'Rows Skipped (insufficient data)', type: 'number' },
      { code: 'findings', label: 'Findings', type: 'data_table', description: 'Subset of recalc_table where pass_fail = fail. Ready to feed straight to a Team Review / Conclude step.' },
      { code: 'pass_fail', label: 'Overall Result', type: 'pass_fail' },
    ],
  },
