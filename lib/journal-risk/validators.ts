 ...
 */
export function validateJournals(journals: JournalRecord[]): void {
 ...
 */
export function validateUsers(users: UserRecord[]): void {
 ...
 */
export function validateAccounts(accounts: AccountRecord[]): void {
...
export function validateConfig(config: Config): void {
  ...
  // Required string fields
  const requiredStrings: (keyof Config)[] = [
    'version',
    'timezone',
    'periodStartDate',
    'periodEndDate',
  ...
  if (isNaN(cutoff.getTime())) {
    throw new Error(
      `Config: "postCloseCutoffDate" is not a valid date: "${config.postCloseCutoffDate}"`
    );
  }
  if (start >= end) {
  ...
  if (
    !config.selection.layer2CoverageTargets ||
