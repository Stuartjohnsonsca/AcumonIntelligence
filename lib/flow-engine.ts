...
async function handleFetchEvidenceOrPortal(
  ...
  // ─── Dedup removed — always fetch fresh from Xero ───
  // Previous dedup logic caused issues: stuck on item 1, stale assessments.
  // Each execution always fetches fresh evidence for reliable results.
        ...
        if (invoice) {
          ...
          const doc = await prisma.auditDocument.create({
            data: {
              ...
              mimeType,
              uploadedDate: new Date(),
              receivedByName: 'Xero (' + connection.system + ')',
              receivedAt: new Date(),
              verifiedOn: new Date(),
