...
export function ScreenCaptureModal({ engagementId, stepId, onCapture, onClose }: Props) {
  ...
  // Waiting phase — snip prompt with auto-detect
  if (phase === 'waiting') {
    return (
      <div className="fixed inset-0 z-[9999] bg-black/70 flex items-center justify-center" onClick={onClose}>
        <div className="bg-slate-900 rounded-2xl shadow-2xl w-[380px] overflow-hidden text-center" onClick={e => e.stopPropagation()}>
          <div className="p-8">
            <div className="w-16 h-16 rounded-full bg-blue-500/20 flex items-center justify-center mx-auto mb-4">
              <Scissors className="h-8 w-8 text-blue-400" />
            </div>
            <p className="text-white text-lg font-semibold mb-2">Snip your screen</p>
            <p className="text-slate-400 text-sm mb-4">Press <kbd className="px-2 py-1 bg-slate-700 rounded text-xs font-mono text-white">{shortcutKey}</kbd> and select the area</p>
